...
## 22. ¶PTF_PRELOAD_EXPAND_CYCLE — a §-reference cycle among queued files recurses forever
`preload_ensure` and `_auto_expand_refs` (in `lib.sh`) are mutually recursive: `preload_ensure` ends by calling `_auto_expand_refs` (line ~1254), which calls `preload_ensure … "next"` for each discovered `§(CMD|FMT|INV)_` ref (line ~1283). The recursion's visited-set is `.preloadedFiles` (checked by `_is_already_preloaded` and by `resolve_refs`'s dedup) — but the `"next"`/queue branch records a path to `.pendingPreloads`, **not** `.preloadedFiles`. So when two not-yet-delivered files reference each other (A cites §CMD_B, B cites §CMD_A), neither is ever marked visited, and the pair recurses without bound. These are **direct** function calls (not `$(…)` subshells), so the bash call stack grows in **one** process → 100% CPU indefinitely. Each frame's `done <<< "$refs"` here-string leaks an `sh-thd-*` temp FD (held until the never-returning function returns) — so the runaway shows BOTH a pegged core and thousands of open (mostly-deleted) here-doc FDs. Same structural fault as #19, escalated from "stale re-delivery" to "infinite recursion".

**The trap**: `sample <pid>` shows only bash-internal addresses in a deep self-similar cycle (no external command), which reads like "a leak" but is a control-flow recursion. And an already-running hook process holds the OLD function definitions in memory — editing `lib.sh` on disk fixes only NEW invocations; a live runaway must be killed. (Orphaned runaway hooks reparent to PID 1 and their subshell children survive the parent's death — kill the whole tree leaf-first.)

**Fix**: a process-global re-entrancy guard at the top of `_auto_expand_refs` — `case "${_EXPANDED_REFS:-}" in *"|${file}|"*) return 0;; esac; _EXPANDED_REFS="${_EXPANDED_REFS:-}|${file}|"`. Expand each file at most once per hook process; bounds recursion to the count of distinct files. bash-3.2-safe (no assoc arrays), fail-safe (a broken/empty var can't loop). Correct semantics — a file's refs only need expanding once.

**Discovered**: 2026-08-07 — one instance had run 6d03h at 99% CPU with 3040 held `sh-thd-*` FDs. Reproduced hermetically with an A↔B §CMD cycle (unpatched: SIGXCPU-killed hang; patched: terminates, all files queued once; acyclic A→B→C still fully expands).
