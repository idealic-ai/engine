...
### Never symlink mutable scripts in test sandboxes — `cat > symlink` follows the link and destroys the real file
**Context**: `setup_test_env()` in test-helpers.sh creates an isolated fake HOME and populates it with scripts for hook tests to call. Previously, `session.sh` was symlinked (`ln -sf`) from the fake home to the real engine script.
**Trap**: If ANY test subsequently writes a mock to `$HOME/.claude/scripts/session.sh` (e.g., `cat > "$HOME/.claude/scripts/session.sh" <<MOCK`), bash's `open()` follows the symlink and overwrites the **real** session.sh with mock content. The damage persists after the test exits because the real file was modified in-place. This destroyed session.sh (2222 lines → 5-line stub) during a 2026-02-13 implementation session.
**Mitigation**: Use `cp` instead of `ln -sf` for any script that tests might overwrite with mocks. The copy lives in the fake HOME and can be safely destroyed. `session.sh` uses `$HOME`-based paths (not `dirname $0`), so copies work identically. Scripts that are only read/sourced (like `lib.sh`) can remain symlinked. Applied fix: `test-helpers.sh:310` changed from `ln -sf` to `cp` for session.sh.
