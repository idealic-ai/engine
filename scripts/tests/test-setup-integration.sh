#!/bin/bash
# ============================================================================
# test-setup-integration.sh — Integration tests for setup.sh
# ============================================================================
# Tests setup.sh end-to-end in a sandboxed environment.
...
#
# Strategy: Override HOME to a temp dir, place setup.sh + libs in a fake
...
create_sandbox() {
  ...
  # This is the key: setup.sh will be run FROM this path, so SCRIPT_DIR resolves here
  cp "$ENGINE_DIR/setup.sh" "$GDRIVE_BASE/engine/scripts/setup.sh" 2>/dev/null || true
  ...
  # Wait — setup.sh sources from SCRIPT_DIR, so we need the scripts dir
  mkdir -p "$GDRIVE_BASE/engine/scripts"
  cp "$ENGINE_DIR/setup.sh" "$GDRIVE_BASE/engine/scripts/"
  ...
  echo "# Test" > "$GDRIVE_BASE/engine/standards/INVARIANTS.md"
...
USERINFO
  ...
  SETUP_SH="$GDRIVE_BASE/engine/scripts/setup.sh"
  ...
  mkdir -p "$PROJECT_DIR"
  # CRITICAL: Export PROJECT_ROOT so setup.sh never uses real pwd
...
# ============================================================================
# Test: setup.sh sources libs without error
# ============================================================================
...
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" status 2>&1 || true)
# The key is: it should NOT fail with "source: file not found" errors
if echo "$OUT" | grep -qi "source.*not found\|No such file"; then
  fail "SRC-01: setup.sh sources libs without file-not-found errors" "no source errors" "$OUT"
else
  pass "SRC-01: setup.sh sources libs without file-not-found errors"
fi
destroy_sandbox

# ============================================================================
# Test: setup.sh local subcommand
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" local 2>&1)
...
# ============================================================================
# Test: setup.sh remote subcommand
# ============================================================================
echo ""
...
create_sandbox
echo "local" > "$FAKE_LOCAL_ENGINE/.mode"
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" remote 2>&1)
...
# ============================================================================
# Test: setup.sh status subcommand
# ============================================================================
echo ""
...
(cd "$PROJECT_DIR" && bash "$SETUP_SH" local >/dev/null 2>&1)
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" status 2>&1)
...
create_sandbox
echo "local" > "$FAKE_LOCAL_ENGINE/.mode"
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" project 2>&1)
...
OUT2=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" project 2>&1)
assert_contains "up to date" "$OUT2" "MIG-04: second run reports migrations up to date"
destroy_sandbox

# ============================================================================
# Test: setup.sh uses lib's parameterized functions (not inline)
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && timeout 10 bash "$SETUP_SH" local 2>&1)
RC=$?

if [ "$RC" -ne 124 ]; then
  pass "LIB-01: setup.sh doesn't hang on real dir (lib's non-interactive mode)"
else
  fail "LIB-01: setup.sh timed out" "non-interactive skip" "hung waiting for input"
fi
...
# ============================================================================
# Test: setup.sh deploy subcommand
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" deploy 2>&1)
...
# Copy setup.sh + libs to a stable location before removing GDrive
STABLE_SETUP="$TEST_ROOT/stable-setup.sh"
cp "$SETUP_SH" "$STABLE_SETUP"
cp "$(dirname "$SETUP_SH")/setup-lib.sh" "$TEST_ROOT/setup-lib.sh"
cp "$(dirname "$SETUP_SH")/setup-migrations.sh" "$TEST_ROOT/setup-migrations.sh"
...
# ============================================================================
# Test: setup.sh push subcommand (Git-based)
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" push 2>&1 || true)
...
# PUSH-02: push without .git → error message mentioning setup.sh local
create_sandbox
echo "local" > "$FAKE_LOCAL_ENGINE/.mode"
...
assert_contains "local" "$OUT" "PUSH-02b: push without .git mentions setup.sh local"
destroy_sandbox

# ============================================================================
# Test: setup.sh pull subcommand (Git-based)
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && bash "$SETUP_SH" pull 2>&1 || true)
...
# PULL-02: pull without .git → error message mentioning setup.sh local
create_sandbox
echo "local" > "$FAKE_LOCAL_ENGINE/.mode"
rm -rf "$FAKE_LOCAL_ENGINE/.git"
...
assert_contains "local" "$OUT" "PULL-02b: pull without .git mentions setup.sh local"
destroy_sandbox
...
# ============================================================================
# Test: setup.sh local subcommand — Git onboarding
# ============================================================================
echo ""
...
OUT=$(cd "$PROJECT_DIR" && echo "" | bash "$SETUP_SH" local 2>&1)
