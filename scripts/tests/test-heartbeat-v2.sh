#!/bin/bash
# tests/test-heartbeat-v2.sh — Tests for pre-tool-use-heartbeat-v2.sh hook
#
# Tests:
...
#   C8. Block at block threshold with tmux escalation attempt
...
HOOK="$HOME/.claude/engine/hooks/pre-tool-use-heartbeat-v2.sh"
...
ln -sf "$REAL_HOOK" "$FAKE_HOME/.claude/hooks/pre-tool-use-heartbeat-v2.sh"
...
RESOLVED_HOOK="$FAKE_HOME/.claude/hooks/pre-tool-use-heartbeat-v2.sh"
...
reset_state() {
  jq '.lifecycle = "active" | .loading = false | .contextUsage = 0 |
      .toolUseWithoutLogsWarnAfter = 3 | .toolUseWithoutLogsBlockAfter = 6 |
...
assert_contains "LOG_BETWEEN_TOOL_USES" "$REASON" "C7: warn message includes log reminder"

# ============================================================
# C8: Block at block threshold
# ============================================================
reset_state
TKEY=$(basename "$TRANSCRIPT_PATH")
jq --arg key "$TKEY" '.toolCallsByTranscript[$key] = 5' "$TEST_SESSION/.state.json" > "$TEST_SESSION/.state.json.tmp" \
