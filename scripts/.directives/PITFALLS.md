...
**Context**: `engine intake doctor` verifies credentials that `engine slack-post` / `engine project fetch` later consume. Widening where the doctor looks (adding a dotfile, adding a global location) feels strictly safer — it can only find *more*.
