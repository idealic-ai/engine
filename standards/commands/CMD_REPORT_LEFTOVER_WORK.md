...
1.  **Identify Artifacts**: Locate the session's debrief, log, and plan files:
    *   Debrief: `[sessionDir]/*.md` matching skill type (e.g., `IMPLEMENTATION.md`, `ANALYSIS.md`, `DOC_UPDATE.md`)
...
2.  **Extract Leftover Items** (from each source):
    *   **From Debrief**:
        ...
        *   Open questions: Items tagged with `#needs-decision` or `#needs-research`
    *   **From Log**:
