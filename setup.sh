...
#
# Examples:
#   ~/Library/CloudStorage/GoogleDrive-.../engine/setup.sh        # infers project from cwd
#   ~/Library/CloudStorage/GoogleDrive-.../engine/setup.sh finch  # explicit project name
#
# Prerequisites:
...
link_if_needed "$ENGINE_DIR/standards" "$CLAUDE_DIR/standards" "standards"
...
if [ -f "$PROJECT_INVARIANTS" ]; then
  echo "  .claude/standards/INVARIANTS.md: already exists"
else
...
STDINV
  echo "  Created .claude/standards/INVARIANTS.md (starter template)"
fi
...
echo "Setup complete!"
echo ""
echo "  Engine:    ~/.claude/{commands,skills,standards,scripts,agents,hooks}"
