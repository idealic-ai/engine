...
**The file is the shareable deliverable (`§INV_ARTIFACT_URL_IS_PRIVATE_PREVIEW`).** The durable, shareable, attachable proof is the on-disk **`<proofPath>`** (`builds/<slug>_PROOF.html`) — self-contained, so it opens anywhere and attaches to a ticket as-is. The **Artifact** tool's `claude.ai/…/artifact/…` URL is a **private, author-only preview** (Artifacts start private; a teammate or a Linear reader CANNOT open it unless the user explicitly shares from the artifact page). So the file outranks the URL: never hand the claude.ai URL to a ticket, a teammate, or a downstream skill as "the proof" — hand the **file** (and, for Linear, the attachment).

**Single generation (`¶INV_PROVE_COMPOSE_ONCE`).** Whatever you publish, publish the **already-composed** `<proofPath>` from `builds/` — pass its file path to the **Artifact** tool, which reads the file from disk. The page is **published, never re-generated**: the subagent composed it once in §2, and that same self-contained file is both the Artifact source and the on-disk deliverable. Do NOT re-render or re-emit the HTML for the publish step. `Skill(artifact-design)` was already loaded by the subagent. Give it an honest `<title>`, a one-sentence `description` that states what the page proves, and an honest favicon.

**Pick the publish route by share-target:**
- **Bound for a ticket / teammate** (chained from `/snapshot`, `/communicate`, `/inbox-post`, `/pr`, or the user says "attach / share / put it on the ticket"): the primary action is **attach the file to Linear** — `prepare_attachment_upload(issue, filename, "text/html", size)` → PUT the raw bytes to the signed `uploadRequest.url` with its headers verbatim (`curl --data-binary @<proofPath>`, 60s window) → `create_attachment_from_upload(issue, assetUrl, title, subtitle)`. Reference *that attachment* in any comment. A claude.ai preview is optional here, and if you mention it, label it "private to you." Do NOT paste the claude.ai URL into ticket/comment bodies.
- **Author-only quick view** (the user just wants to see it): publish to the **Artifact** tool and hand over the URL — but say plainly it is **private until shared**, and that the shareable copy is the file in `builds/`.
...
**Report:** lead with the durable artifacts in `builds/` — the **proof HTML** (`<trailDir>/<slug>_PROOF.html`, self-contained; the shareable deliverable, attach it directly to a Linear ticket via the MCP upload flow above, or let `/snapshot`//`/communicate` surface it) and its dossier (`<trailDir>/<slug>_PROVE.md`) (`§CMD_LINK_FILE` each). Only THEN, if you published one, the Artifact URL — explicitly marked *author-only preview, private until shared*. Close with a one-line honesty summary: `Shown: N claims with real assets · trusted-from-upstream: N (labeled) · out-of-scope: …`
...
- **`¶INV_ARTIFACT_URL_IS_PRIVATE_PREVIEW` — the file is what's shareable, not the claude.ai URL.** A `claude.ai/…/artifact/…` URL from the **Artifact** tool is a **private, author-only preview** — Artifacts start private, and a teammate or a Linear reader cannot open it unless the author explicitly shares it from the artifact page. So anything that must leave the author's own view — a ticket comment, an attachment, a handoff to a teammate or a downstream skill (`/inbox-post`, `/snapshot`, `/communicate`, `/pr`) — carries the **self-contained file** (`builds/<slug>_PROOF.html`), attached as a real file (Linear MCP `prepare_attachment_upload` → PUT bytes → `create_attachment_from_upload`), never the private preview URL pasted as "the proof." Publish the claude.ai Artifact only as an optional author-only preview, and label it *private until shared* when you do. Applies to every skill that forwards a proof/report, not just `/prove`.
