...
*   **It is** a **parallel-thread orchestrator**. A *thread* is one ticket walking the full chain; many threads run at once. `/loom` composes the building-block skills — it dispatches `/inbox-triangulate`, `/ticket`, `/communicate`, `/interrogate`, `/probe`, `/experiment`, `/build`, `/scrutinize`, `/snapshot`, `/pr`, `/council`, `/inbox-post`, `/inbox-next`, `/ticket-search` — and its own work is the conducting: partitioning files, relaying agent messages, brokering conflicts, detecting deadlocks, escalating, and keeping the ledger.
