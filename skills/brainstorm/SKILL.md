...
Before closing, offer a `/council` panel review on the brainstorm `<BRAINSTORM.md>` via `§CMD_OFFER_COUNCIL_REVIEW` (offer, not force) -- diverse expert eyes on the ideas before you act on them. Then offer a `/graph` flowgraph of the decision tree / option structure of the design via `§CMD_OFFER_GRAPH_VIZ` (offer, context-gated -- skips linear artifacts).
