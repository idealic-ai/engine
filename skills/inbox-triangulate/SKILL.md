...
*   **Data access**: the project's shared read-only connection, already open. Both runs share it; neither opens its own — **so both must be TOLD the endpoint**, in the handoff, by name.
*   **The access directive itself** — `.directives/INBOX_TRIAGE_ACCESS.md`. Nothing auto-loads it and `discover-directives` does not return it, so **name the path in both briefs**. An angle that never reads it does not fail loudly: it goes looking for a route, finds the fallback, and reports a confident diagnosis of the wrong thing. That is not a hypothetical — it has happened, against a public database instance gated by an IP allowlist whose failure mode is a silent timeout.
...
**Build ONE dossier and link it once — beside the synthesis, not instead of it** (`TEMPLATE_TICKET_COMMENT.md` has the per-reader table; the dossier is the *auditor's* card, and the reader who owns the bug gets the readable report as its own). Concatenate — in this order, with a table of contents — the adjudication (or, where no adjudicator ran, a short note saying the two angles agreed), angle A's report, angle B's report, and the context pack, into `builds/inbox-triage-<origin-id>-dossier.md`. Upload that single file and embed it as a clickable `[📄 filename](assetUrl)` link **alone in its own block with nothing after it** so Linear renders a preview card (`§INV_CHANNEL_EVIDENCE_RIDES_THE_COMMENT` — naming a file in prose is not linking it). **One dossier, not four fragments**: the workings are one reader's question, and whoever wants them wants all of them at once — so the adjudication and both angle reports travel together rather than as competing cards. That is a rule about *this* card's contents, not a cap on how many cards the comment carries; the synthesis and each angle's repro are different readers and get their own.
...
**Upload each angle's repro script as its own attachment — one per angle, never merged.** The dossier stays one card because it is prose; a script is executable and is read by whoever takes the fix, not by the reporter. Two angles that both drove the app produce **two** attachments, each named for its angle (`…-<origin-id>-<angle>-repro.ts`), because they may reproduce different things at different screens and merging them loses both the attribution and probably one of the repros. **Scripts are TypeScript — never Python** (`/inbox-triage` §Reproduce carries the reasoning); an angle that returns a `.py` did not follow the protocol, so say so when you upload it rather than passing it on silently. The angles run `--no-post` and upload nothing themselves — they return their paths, and **this wrapper uploads them** alongside the single dossier link. An angle that wrote no script contributes no attachment; say so in one clause rather than leaving the reader to wonder.
