...
**Upload each angle's repro script as its own attachment — one per angle, never merged.** The dossier stays one card because it is prose; a script is executable and is read by whoever takes the fix, not by the reporter. Two angles that both drove the app produce **two** attachments, each named for its angle (`…-<origin-id>-<angle>-repro.<ext>`), because they may reproduce different things at different screens and merging them loses both the attribution and probably one of the repros. The angles run `--no-post` and upload nothing themselves — they return their paths, and **this wrapper uploads them** alongside the single dossier link. An angle that wrote no script contributes no attachment; say so in one clause rather than leaving the reader to wonder.
