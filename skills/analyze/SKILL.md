...
Before closing, offer a `/council` panel review on the report doc `<ANALYSIS.md>` via `§CMD_OFFER_COUNCIL_REVIEW` (offer, not force) -- diverse expert eyes on the findings before they drive decisions. Then offer a `/graph` flowgraph of the analyzed algorithm / control flow / structure via `§CMD_OFFER_GRAPH_VIZ` (offer, context-gated -- skips linear artifacts).
