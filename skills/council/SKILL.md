...
- **Engine Mode (`COMMANDS.md` present):** A session may be active. Use `engine log` and set `<trailDir> = <sessionDir>/builds/`. All `§CMD_*` / `§FMT_*` references resolve to their engine definitions. *Even here, council does not read the session for context — it reads the Brief;* the session only supplies the trail location.
- **Standalone Mode (`COMMANDS.md` absent):** You are assisting a teammate without the engine. No session exists. Use the global `/tmp` trail directory and plain file appends (defined in §1.C). Treat every `§CMD_*` / `§FMT_*` reference below as plain-English guidance (the surrounding prose describes the behavior).
...
- **Standalone Mode:** `<trailDir> = ${TMPDIR:-/tmp}/council-build-trail/<repo-basename>/` (`mkdir -p`).
...
**Also append the composed panel to the running panel log** (a passive audit trail, NEVER a gate). Right after you state the seated panel, append that same `Panel (N): …` line — prefixed with the subject type and followed by the `report_path` — to `<trailDir>/COUNCIL_PANELS.md`, so selection composition can be audited over runs (later you can grep "how often was the Copywriter seated in the last N runs" and catch the selector regressing to the same few personas — a monoculture guard). This **records, it never blocks**: it does not gate, re-open, or alter the panel you just composed. Append by the mode-appropriate path, exactly as §5.E does for `LESSONS.md` — blind append, never read-modify-write (`¶CMD_APPEND_LOG`):
- **Engine Mode** — `engine log <trailDir>/COUNCIL_PANELS.md` with a **`## `-headed block** (it requires the heading and auto-injects the timestamp; a headerless body exits 1). Put the subject type, the `Panel (N): …` line, and the `report_path` as bullets under one dated `## ` heading.
- **Standalone Mode** — no `engine log`; append a single self-contained line: `printf '%s\n' '- <subject type> · Panel (N): … · <report_path>' >> <trailDir>/COUNCIL_PANELS.md` (a lone short `>>` is atomic, so a sibling council's line may safely interleave).
...
>
> **Return contract:** WRITE your findings to `<trailDir>/<slug>_council/<lens>.md` (the orchestrator gives you the fully-substituted absolute path — do NOT hardcode `~/.claude`, and do NOT write to any other path or you will clobber another expert). Structure: for each finding — the tier, a short title, `file:line`, why it's wrong through your lens, the failing scenario, and a suggested direction (not a patch). End with a one-line lens verdict. Then RETURN to the orchestrator a compact list: each finding's tier + title + `file:line` (one line each) + your verdict. Do NOT dump full findings into the return message — the orchestrator reads your file.
...
**Append by the mode-appropriate path — never both, never a third format.** `LESSONS.md` is shared: a backgrounded council may write it while a concurrent `/pr`, `/build`, or another council appends too, so a write must not corrupt a sibling's. Which write to use is decided by execution mode (§1.C), and the two paths are NOT interchangeable — never read-modify-write the file (`¶CMD_APPEND_LOG` — blind append, always):
- **Engine Mode** — append with `engine log <trailDir>/LESSONS.md` using a **`## `-headed block**. This is exactly `/build`'s convention for the SAME shared file (its ledger step), so the two skills' entries share one format. `engine log` is already append-safe AND *requires* a `## ` heading — it exits 1 without one, and auto-injects the timestamp, per `¶CMD_APPEND_LOG` — so put the settled outcomes as bullets under one dated `## ` heading; do NOT hand `engine log` a headerless body.
- **Standalone Mode** — no `engine log` is available. Append each lesson as its **own** single-line `printf '%s\n' '- <fact>' >> <trailDir>/LESSONS.md`. A lone `>>` write of a short single line is atomic in practice (a multi-line block is not), so a sibling's line may safely land between yours — which is exactly why, on THIS path only, each line must stand alone with no header above it. Never buffer several facts into one multi-line `printf`.
