...
Before closing, offer a `/council` panel review on the vision doc `<the vision doc>` via `§CMD_OFFER_COUNCIL_REVIEW` (offer, not force) -- diverse expert eyes on the direction before chapters kick off.
