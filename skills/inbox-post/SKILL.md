...
*   **Evidence travels as a FILE on the issue (or a public URL), never a private preview (`§INV_PROVE_S3_URL_IS_SHAREABLE`).** When the evidence is a rendered proof / report / screenshot on disk (e.g. `builds/<slug>_PROOF.html`, a PNG), plan to **upload the file to the issue** in step 4 and reference the attachment in the body. A `/prove` proof also has a public **S3 URL** (uploaded via `assets/publish-s3.sh`) a teammate can open directly — reference that if you have it. Either way the reader must be able to open the evidence without the author's session.
...
*   **Attach the evidence files first (if any).** For each on-disk proof/screenshot/report the item leans on: `prepare_attachment_upload(issue, filename, contentType, size)` → PUT the raw bytes to the signed `uploadRequest.url` with its headers verbatim (`curl --data-binary @<path>`, 60s window) → `create_attachment_from_upload(issue, assetUrl, title, subtitle)`. Then reference that attachment by title in the comment body. If the evidence is a `/prove` proof with a public **S3 URL**, referencing that URL in the body is also fine (`§INV_PROVE_S3_URL_IS_SHAREABLE`) — the point is the reader can open it without the author's session.
