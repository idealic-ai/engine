...
**What goes in the handbook**: the 📋 report template · what happens next (including *"the triage that comes back is a draft"*) · the other-intake-inboxes list · **what triage will chase, written for THIS project** · how a comment becomes a ticket. Every section but that one is shared text; that one names this project's own tools, tables and hosts.
...
**The two-event Slack, synced with Project Updates** (`§PASS_HEARTBEAT`): the notification ties to the **Project Update** at two wave points. The **Linear Update evolves in place** (edited); the **Slack side is two fresh posts** (each pings). Linear and Slack are independent transports, kept decoupled so one's failure never strands the other:
1. **Event 1 — board prepared** (Phase 5): post a Project Update ("ingested `<N>` · Decision Board: `<url>`") + a fresh Slack announce; **capture the `save_status_update` id** into `INTAKE.md`'s pinned `Project Update:` slot — the id, not the phase, so Close can find it after a rehydrate; capture wherever the Update actually posts.
2. **Event 2 — wave done** (Phase 7 Close): render the Outcomes Board, then **edit** that same Update (`save_status_update` with the captured id) to add the outcomes + the Outcomes Board URL, and post a **FRESH** Slack announce ("ingestion complete — `<N>` filed / `<M>` merged / `<K>` adopted · Outcomes Board: `<url>`") — a new message, because an edited Slack message pings no one and the heartbeat exists to make a *completed* pass **visible**.

So the Linear Update resolves in place while completion gets its own broadcast. **Degraded fallback — fires on a MISSING id OR a FAILED edit**: if no id was captured (the Update was never posted) or the `save_status_update` edit errors (the update was deleted), Close posts a **fresh** Project Update instead — a failed edit never silently swallows the checkpoint. (`slack-post` also carries a general `--update-ts`/`chat.update` mode — edits a prior message, returns the posted `ts` for capture — but the intake flow uses fresh Slack posts and does not rely on it; it is there for any caller that wants edit-in-place Slack.)
...
- **Setup** (one-time): the poster authenticates with a Slack **bot token** (`chat:write` scope, bot invited to the target channel). Provide it out-of-band, never in chat/committed source:
  ...
  - `SLACK_INTAKE_CHANNEL` — **the channel ID (`C…`), never the `#name`.** Two separate traps here, both found the hard way; a name works right up until the moment the two-event flow needs it most.
    - **Use the ID.** `chat.postMessage` accepts a `#name`, but **`chat.update` rejects it with `channel_not_found`** — so a name-configured project posts event 1 happily and then **cannot edit it at event 2**, which is exactly when the announce turns from "in progress" to "complete". An ID satisfies both endpoints, so it is the only value that makes the two-event announce whole. Find it in Slack under *channel name → About → Channel ID*, or as the `C…` segment of the channel URL. And the poster **cannot resolve a name itself** — `conversations.list` needs `channels:read`, which a `chat:write`-only bot token lacks.
    - **The env file does not carry it.** `--env-file` key-extracts the *token only*; the channel comes from `--channel` or an **exported** `$SLACK_INTAKE_CHANNEL`. Putting it in the env file alone leaves it **empty**, and the failure is quiet — `--dry-run` shows `"channel": ""`. Pass `--channel` explicitly unless you know the var is exported.
    - The channel is per-project (like `PROVE_S3_BUCKET`), so prescribe it in the project's own config, never in this shared catalog.
- **Best-effort**: a missing token/channel or a Slack `error` (e.g. `not_in_channel` → `/invite` the bot) is logged and **never blocks the wave's Close**. `engine slack-post --dry-run` prints the exact payload (token redacted) for a safe check — **run it once before the first real post on a project**, since it is the only way to see the resolved channel before it goes out.
