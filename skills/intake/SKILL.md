...
*   **Event 2 of the two-point announce — edit the Project Update in place, and post a FRESH completion ping** (`§PASS_HEARTBEAT`): read the `save_status_update` id from `INTAKE.md` (§1), then **edit that same Update** (`save_status_update` with the id) to add the outcomes + the **Outcomes Board URL** — so the Linear Update evolves in place. Then **post a FRESH Slack announce** ("ingestion complete — `<N>` filed / `<M>` merged / `<K>` adopted · Outcomes Board: `<url>`") — a **new message, not an edit**: an edited Slack message pings no one, and the whole point of the heartbeat is to make a *completed* pass visible, so completion gets its own broadcast. **Degraded fallback — fire on a MISSING id OR a FAILED edit**: if no id was captured (the Update was never posted) OR the `save_status_update` edit errors (e.g. the update was deleted), post a **fresh** Project Update instead — never let a failed edit silently swallow the checkpoint. Offered/confirmed as always; best-effort, never blocks Close.
