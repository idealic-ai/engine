...
import { sendQuery, type QueryResult } from "../../../db/src/query-client.js";
...
describe("integration: v3 lifecycle via raw SQL", () => {
  ...
  it("should store and retrieve JSONB metadata on efforts", async () => {
    ...
    await q("INSERT INTO efforts (task_id, skill, ordinal, metadata) VALUES (?, ?, ?, json(?))", ["t1", "impl", 1, metadata]);
