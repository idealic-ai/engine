...
2.  **Card → question body (plain text, `Label: value` per section).** Put the card's *analysis* in the `question` field — heading (`itemId · Title`), a subtitle (`scope · △●Ⓜ`), What's-at-stake, How-to-verify, My-lean, Confidence. **No markdown** — AskUserQuestion bodies render `**bold**`/`` `code` ``/`####` literally; render each field as `Label: value` inline (plain labels, never bold) with a **blank line between sections** — **not** whitespace-aligned columns (the padding gap is hard to scan). **Do NOT list the Options in the body** — the answers ARE the options (Step 3), so re-listing is redundant. `header` = the item ID (SIGILS.md § Item IDs).
