...
```json
{
  ...
  "properties": {
    ...
    "taskType": {
      ...
      "enum": ["ANALYSIS", "BRAINSTORM", "CHANGESET", "CHORES", "DEBUG", "DIVERGE", "DOC_IMPROVE", "DOCUMENT_UPDATE", "EVANGELISM", "IMPLEMENTATION", "RESOLVE", "TESTING"],
    ...
    "provableDebriefItems": {
      "type": "array",
      "items": { "type": "string" },
      "title": "Provable Debrief Items (DEPRECATED)",
      "description": "DEPRECATED: Replaced by proof-gated phase transitions. Proof is now provided inline with each sub-phase transition via `proof` fields in the `phases` array. This field and `engine session prove` are kept for backward compatibility but will be removed in a future version. New skills should use `proof` fields on synthesis sub-phases instead.",
      "example": [
...
1.  Execute `§CMD_GENERATE_DEBRIEF_USING_TEMPLATE` — creates the debrief file (steps 1-7 of that command).
2.  Prove:
    ```bash
...
1.  Execute `§CMD_REPORT_RESULTING_ARTIFACTS` — list all created/modified files in chat.
...
4.  Prove:
    ...
    EOF
    ```
5.  Execute `§CMD_DEACTIVATE_AND_PROMPT_NEXT_SKILL` — deactivate session, present next-skill menu.
...
**On "Proceed to next phase"**: After the exit gate resolves, fire `§CMD_TRANSITION_PHASE_WITH_OPTIONAL_WALKTHROUGH`. This gives the user the walkthrough option (review what was established during interrogation) before committing to the next phase. Current/next/previous phases are derived from the `phases` array in `.state.json`.
...
**Trigger**: After plan approval in plan-based skills (implement, fix, test, document). Extends `§CMD_HAND_OFF_TO_AGENT` with multi-agent coordination.
...
**Trigger**: Called by `§CMD_GENERATE_DEBRIEF_USING_TEMPLATE` step 10 (after `§CMD_PROCESS_DELEGATIONS`, before `§CMD_CAPTURE_SIDE_DISCOVERIES`). Also callable standalone via `/delegation-review`.
