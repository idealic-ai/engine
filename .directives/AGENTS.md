...
*   **¶INV_MODIFIED_SINCE_READ_IS_A_HARD_SIGNAL**: A *"File has been modified since read"* refusal means **another writer touched this file** — in a shared engine, usually a lane still running. It is not bookkeeping.
    *   **Rule**: The refusal tells you the file changed; it does **not** tell you *where*. So a partial re-read is worse than none — it manufactures confidence. Either **re-read the file in full** before replacing it, or **stop replacing and edit surgically** so the parts you never read are the parts you never touch.
    *   **The tell**: re-reading the first N lines, recognising them, and concluding "unchanged". Familiar text at the top is evidence about the top and nothing else. If you are about to write *"unchanged — just needed a re-read for the tool"*, you are about to overwrite something you have not seen.
    *   **Weigh the blast radius, not the diff size.** Whole-file `Write` destroys everything you didn't read; `Edit` cannot. On a shared file the default should be `Edit`, and a full replace should be a decision rather than a habit.
    *   **Check the recovery path before you assume there isn't one — and check it at the right level.** `~/.claude` is NOT a git repo, but **`~/.claude/engine` IS**, and most engine files are tracked there. `git -C ~/.claude/engine log -- <path>` and `git show <sha>:<path>` are real recovery. What is genuinely unrecoverable is a file with **no commit yet** — a `git log` returning nothing means the working copy is the only copy, and that is the case to treat as one-way.
    *   **The generalisation is the trap, not the fact.** Testing `~/.claude` and concluding "the engine has no version control" is one `-C` away from correct and produces exactly the wrong risk model: it makes safe edits feel doomed and, worse, gets written down as fact. Test the directory you are about to write to, not its parent.
    *   **Reason**: captured from a real incident. A session was told the file had changed, re-read 8 lines of ~80, logged "unchanged in content", and full-file-replaced a shared contract — destroying a sibling lane's just-landed class. The rewrite was the better design and that was beside the point; it was never read. It survived only because two other sessions had independently written down what it was.
