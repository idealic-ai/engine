...
**Behavioral norms (~70%)**: The commands, invariants, and conventions in the standards documents — over a thousand lines of behavioral instructions across COMMANDS.md, INVARIANTS.md, and SIGILS.md. Fully voluntary compliance. The agent follows these because they're loaded into context and because the training signal from well-structured instructions is strong — but there's no mechanical enforcement. If the agent ignores a behavioral norm, nothing blocks it.
