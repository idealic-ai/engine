...
**Definition**: During synthesis, processes all discovered CHECKLIST.md files — reads each checklist, evaluates items against the session's work, then quotes results back to `session.sh check` for mechanical validation. Ensures the deactivation gate (`¶INV_CHECKLIST_BEFORE_CLOSE`) will pass.
**Trigger**: Called by skill protocols during the synthesis phase, BEFORE `§CMD_GENERATE_DEBRIEF_USING_TEMPLATE`. Read this file before executing.
...
4.  **Submit to session.sh check**: Concatenate all quote-back blocks and pipe to `session.sh check`:
    ...
    EOF
    ```
    *   On success: `session.sh check` sets `checkPassed=true` in `.state.json`. The deactivation gate will pass.
    *   On failure: `session.sh check` exits 1 with a descriptive error. Fix the missing blocks and retry.
...
*   **Quote-back pattern**: The agent MUST echo checklist items back via stdin to prove processing. `session.sh check` validates that every `discoveredChecklists[]` path has a matching `## CHECKLIST:` block with at least one item.
*   **Belt-and-suspenders**: This command is the "belt" (protocol-level). The `session.sh deactivate` gate is the "suspenders" (infrastructure-level). Both exist because agents skip protocol steps — the gate catches failures.
